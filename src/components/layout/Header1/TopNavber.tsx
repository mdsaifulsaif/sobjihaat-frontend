...
//       ) : (
//         <div className="p-4">
//           <h4 className="text-sm font-semibold text-[var(--color-text-primary)] mb-0.5">Welcome!</h4>
//           <p className="text-xs text-[var(--color-text-muted)] mb-3">Access your account</p>
//           <div className="flex gap-2">
//             <Link href="/login" className="flex-1 py-2 text-center text-xs font-bold bg-[var(--color-primary)] text-white rounded-lg hover:opacity-90 transition-opacity">
//               Sign in
//             </Link>
//             <Link href="/register" className="flex-1 py-2 text-center text-xs font-bold border border-gray-200 text-[var(--color-text-secondary)] rounded-lg hover:border-[var(--color-primary)] hover:text-[var(--color-primary)] transition-all">
//               Join
//             </Link>
//           </div>
//         </div>
...
//           <Link href="/dashboard/admin" className="flex items-center gap-2 px-4 py-2.5 text-sm font-semibold text-[var(--color-primary)] bg-[var(--color-primary)]/5 hover:bg-[var(--color-primary)]/10 transition-all border-b border-gray-100">
//             💎 Admin Dashboard
//           </Link>
//         )}
//         {[
...
//           { label: "Support", href: "#" },
//         ].map((item) => (
//           <Link key={item.href} href={item.href} className="block px-4 py-2.5 text-sm text-[var(--color-text-secondary)] hover:bg-[var(--color-surface)] hover:text-[var(--color-primary)] transition-all">
//             {item.label}
//           </Link>
//         ))}
//         {isAuthenticated && (
...
//                 <UserDropdown />
//               </div>
//               <Link href="/wishlist" className="w-[42px] h-[42px] border border-gray-200 rounded-lg flex items-center justify-center hover:border-[var(--color-primary)] transition-colors relative group">
...
//             {/* Row 1: hamburger + logo + location + login + heart */}
//             <div className="flex items-center gap-2">

//               {/* ✅ Mobile hamburger — home route এও */}
//               <button
//                 onClick={onToggleMobile}
//                 className="w-[36px] h-[36px] flex items-center justify-center border border-gray-200 rounded-lg hover:bg-[var(--color-surface)] transition-colors flex-shrink-0"
...
//                 <button className="flex items-center gap-1 px-2.5 h-[36px] border border-gray-200 rounded-lg bg-[var(--color-surface)] text-[12px] text-[var(--color-text-secondary)] whitespace-nowrap">
...
//                 <div className="relative group">
//                   <button className="flex items-center gap-1 px-2.5 h-[36px] border border-gray-200 rounded-lg text-[12px] text-[var(--color-text-secondary)] whitespace-nowrap">
//                     {isAuthenticated ? (
...
//                 <Link href="/wishlist" className="w-[36px] h-[36px] border border-gray-200 rounded-lg flex items-center justify-center relative flex-shrink-0">
...
//                 <p className="text-[10px] font-semibold text-[var(--color-text-muted)] uppercase leading-none tracking-wide">Account</p>
    ...
    <header className="bg-white border-b border-gray-100 sticky top-0 z-40 flex-shrink-0">
      {isHome ? (
        <>
          ...
          <div className="container mx-auto px-4 h-[68px] hidden md:flex items-center gap-3">
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              ...
              >
                <FiHeart
                  size={17}
                  className="text-[var(--color-text-secondary)] group-hover:text-red-500 transition-colors"
                />
                {wishlistItems.length > 0 && (
                  <span className="absolute top-1.5 right-1.5 w-4 h-4 bg-red-500 text-white text-[9px] rounded-full flex items-center justify-center font-bold border border-white">
                    {wishlistItems.length}
          ...
          {/* ── HOME MOBILE ── (সব রুটে একই) */}
          <div className="flex md:hidden flex-col px-4 py-3 gap-2.5">
            {/* Row 1: Hamburger + Logo + Right Side Icons */}
            <div className="flex items-center gap-2">
              {/* Hamburger */}
              <button
                onClick={onToggleMobile}
              ...
              {/* Right Side: Location + User + Wishlist */}
              <div className="flex items-center gap-1.5 ml-auto flex-shrink-0">
                {/* Location Button */}
                <button className="flex items-center gap-1 px-2.5 h-[36px] border border-gray-200 rounded-lg bg-[var(--color-surface)] text-[12px] text-[var(--color-text-secondary)] whitespace-nowrap">
                  <FiMapPin size={12} className="text-[var(--color-primary)]" />
                ...
                <div className="relative group">
                  <button className="flex items-center gap-1 px-2.5 h-[36px] border border-gray-200 rounded-lg text-[12px] text-[var(--color-text-secondary)] whitespace-nowrap hover:border-[var(--color-primary)] transition-colors">
                    {isAuthenticated ? (
                      <div className="w-5 h-5 rounded bg-[var(--color-primary)]/10 flex items-center justify-center text-[var(--color-primary)] font-bold text-[10px]">
                        {user?.name?.charAt(0).toUpperCase()}
                      </div>
        ...
        <div className="w-full">
          ...
          <div className="w-full px-4 h-[60px] flex items-center gap-3">
            ...
            {/* Desktop এ সার্চবার */}
            <div className="hidden md:flex flex-1 items-center gap-2.5 border border-gray-200 rounded-lg bg-[var(--color-surface)] px-4 h-[40px] focus-within:border-[var(--color-primary)] focus-within:bg-white transition-all">
              <FiSearch
                size={14}
                className="text-[var(--color-text-muted)] flex-shrink-0"
              />
            ...
            {/* Mobile-এ রাইট সাইড আইকন (Location + Wishlist) */}
            <div className="md:hidden flex items-center gap-2 ml-auto">
              <button className="flex items-center gap-1 text-[11px] text-[var(--color-text-secondary)]">
                <FiMapPin size={12} className="text-[var(--color-primary)]" />{" "}
                Dhaka
              </button>
              <Link href="/wishlist" className="relative p-1.5">
                <FiHeart size={16} />
                {wishlistItems.length > 0 && (
                  <span className="absolute top-0 right-0 w-3 h-3 bg-red-500 text-white text-[7px] rounded-full flex items-center justify-center">
                    {wishlistItems.length}
                  </span>
                )}
              </Link>
            ...
            {/* User Profile */}
            <div className="relative group flex-shrink-0 ml-auto md:ml-0">
              <button className="flex items-center gap-2.5 p-1.5 hover:bg-[var(--color-surface)] rounded-lg transition-colors">
                <div className="w-[34px] h-[34px] rounded-lg bg-[var(--color-primary)]/10 flex items-center justify-center text-[var(--color-primary)] text-[13px] font-semibold flex-shrink-0">
                  {isAuthenticated ? (
                    user?.name?.charAt(0).toUpperCase()
                ...
                <div className="hidden lg:block text-left">
                  ...
                  <p className="text-[13px] font-semibold text-[var(--color-text-primary)] mt-0.5">
                    {isAuthenticated ? user?.name?.split(" ")[0] : "Login"}
                  </p>
                </div>
              </button>
          ...
          {/* Row 2: Search Bar for Mobile Only */}
          <div className="md:hidden w-full px-4 pb-3">
            <div className="flex items-center gap-2 border border-gray-200 rounded-lg bg-[var(--color-surface)] px-4 h-[42px] focus-within:border-[var(--color-primary)] focus-within:bg-white transition-all">
              <FiSearch
                size={14}
                className="text-[var(--color-text-muted)] flex-shrink-0"
              />
              ...
              />
              <button className="w-[32px] h-[32px] bg-[var(--color-primary)] rounded-md flex items-center justify-center flex-shrink-0">
                <FiSearch size={13} className="text-white" />
              </button>
            </div>
          </div>
