    ...
    <div className="bg-white border-b border-gray-100 shadow-sm">
      <div className="container mx-auto px-4 sm:px-8 md:px-12 lg:px-16">
        <div className="flex justify-between items-center h-14">
          ...
          <div className="hidden md:flex items-center gap-10 h-full">
            {navLinks.map((link, idx) => (
              <Link
                key={idx}
                href={link.href}
                className={`text-[14px] font-medium transition-all hover:text-[var(--color-primary)] relative py-4 ${
                  isActive(link.href)
          ...
          <Link
            href="/offers"
            className="hidden lg:flex items-center gap-2 text-[var(--color-primary)] font-bold text-sm animate-pulse cursor-pointer group"
          >
            <FiZap size={18} />
