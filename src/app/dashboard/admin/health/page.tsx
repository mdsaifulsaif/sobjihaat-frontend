...
const ApiHealthPage = () => {
  ...
  } = useGetProductsQuery({ limit: 1 });
  const {
    isError: userError,
    isLoading: userLoading,
    refetch: refetchUsers,
  } = useGetAdminUsersQuery({ limit: 1 });
  const {
    isError: statsError,
  ...
  const modules = [
    {
      ...
      desc: "Central backend system status",
    },
    {
      ...
      desc: "Manage inventory and listings",
    },
    {
      ...
      status: !userError,
      loading: userLoading,
      desc: "Authentication and profile data",
    },
    {
      ...
      desc: "Sales tracking and reports",
    },
  ];
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10">
        <div>
          ...
          <p className="text-gray-500 mt-1 font-medium italic">
            Monitoring real-time API connectivity and service status
          </p>
        </div>
        <button
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden mb-8">
        ...
        <div className="p-8 flex flex-col md:flex-row gap-12">
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-6 bg-emerald-50/50 rounded-lg border border-emerald-100">
            <div
              className={`w-16 h-16 rounded-full flex items-center justify-center mb-3 ${healthError ? "bg-red-100 text-red-500" : "bg-emerald-100 text-emerald-500"}`}
            >
              {healthError ? (
            ...
            <p
              className={`text-xl font-black ${healthError ? "text-red-600" : "text-emerald-600"}`}
            >
              {healthError
            ...
            <p className="text-sm text-gray-500 mt-1 font-medium">
              Connection verified with primary database
            </p>
          </div>
        </div>
      </div>
        ...
        {modules.map((module, idx) => (
          ...
          >
            <div className="flex items-start justify-between mb-4">
              ...
              <div
                className={`w-3 h-3 rounded-full ${module.loading ? "bg-yellow-400 animate-pulse" : module.status ? "bg-emerald-400" : "bg-red-400"}`}
              ></div>
            </div>
      ...
      <div className="mt-12 p-6 bg-blue-50/50 rounded-xl border border-blue-100 flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-blue-700 font-medium">
            This page performs real-time verification of all core API
            controllers. If a service shows as 'offline', please check your
