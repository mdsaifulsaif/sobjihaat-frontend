  ...
  const runSingleTest = async (testId: string) => {
    ...
    try {
      const response = await fetch(
        `${baseUrl}${tests.find((t) => t.id === testId)?.endpoint}`,
        {
          headers: {
            Authorization: `${token}`,
    ...
    <div className="p-8 max-w-5xl mx-auto">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
        <div>
          ...
          <p className="text-gray-500 mt-2 font-medium">
            Verify data integrity across all system modules in one click.
          </p>
        </div>
        <button
          onClick={runAllTests}
          disabled={isScanning}
          className={`flex items-center gap-2 px-8 py-4 rounded-md font-bold transition-all shadow-xl active:scale-95 ${
            isScanning
      ...
      <div className="grid gap-4">
        {tests.map((test) => (
          <div
            key={test.id}
            className={`bg-white border rounded-xl p-5 flex flex-col md:flex-row md:items-center justify-between gap-4 transition-all ${
              test.status === "success"
          ...
          >
              ...
              <div>
                ...
                <div className="flex gap-2 mt-1">
                  {test.operations.map((op) => (
                    <span
                      key={op}
                      className="text-[10px] font-black px-2 py-0.5 bg-gray-100 text-gray-400 rounded-full border border-gray-200 uppercase"
                    >
                      {op}
                    </span>
                  ))}
            ...
            <div className="flex-1 px-4 hidden md:block">
              {test.status !== "idle" && (
                <p
                  className={`text-sm font-medium ${test.status === "failed" ? "text-red-500" : "text-gray-400"}`}
                >
                  {test.result ||
                    "Analysing endpoint connectivity and response latency..."}
                </p>
              )}
            </div>
            ...
            <div className="flex items-center gap-6 justify-between md:justify-end">
              ...
              >
                <FiRefreshCw
                  size={18}
                  className={test.status === "running" ? "animate-spin" : ""}
                />
              </button>
            </div>
          </div>
      ...
      <div className="mt-10 p-6 rounded-xl border-2 border-dashed border-gray-200 text-center">
        <p className="text-sm text-gray-400 font-medium">
          This scan performs{" "}
          <span className="text-gray-600">POST, GET, PATCH, and DELETE</span>{" "}
          simulation tests. Successful tests confirm that the database is
          writable and API routes are properly authorized.
        </p>
      </div>
    </div>
  );
