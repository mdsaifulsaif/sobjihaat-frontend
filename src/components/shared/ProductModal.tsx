...
//   const product: Product | null = (productResponse as any)?.data ?? null;

//   const hasVariants = !!(product?.variants && product.variants.length > 0);
//   const [selectedVariant, setSelectedVariant] = useState<Variant | null>(null);

//   // When product loads, pick first variant (if any)
//   useEffect(() => {
...
//   const { data: relatedData, isLoading: relatedLoading } = useGetRelatedProductsQuery(categoryId, {
//     skip: !categoryId,
//   });
...
//   const cartItemId = product ? getCartItemId(product, selectedVariant) : '';
...
//           <p className="text-gray-700 font-medium mb-1">Failed to load product</p>
//           <p className="text-sm text-gray-500 mb-5">
//             {(productFetchError as any)?.data?.message || 'Something went wrong'}
//           </p>
...
//                         // ওজন/ভলিউম বের করা
//                         let weightOrVolume = p?.weightOrVolume || 0;
//                         let unitShortName = p?.unit?.shortName || '';
                        
//                         // যদি variant থাকে এবং তার ওজন/ভলিউম থাকে
//                         if (variant) {
//                           weightOrVolume = variant.weightOrVolume || weightOrVolume;
//                           // variant এর unit ID থেকে unit নাম বের করা (যদি থাকে)
//                           if (variant.unitID && p?.unit) {
//                             unitShortName = p.unit.shortName || unitShortName;
//                           }
//                         }
                        
//                         const unit = p?.unit?.shortName || 'pc';
//                         const itemPrice =
//                           variant?.salePrice ||
...
//                         // আইটেমের নাম তৈরি করা
//                         let itemName = p?.name || '';
//                         if (weightOrVolume > 0 && unit) {
...
//                           <div
//                             key={idx}
//                             className="grid grid-cols-4 gap-2 px-3 py-2 border-t border-gray-100 items-center hover:bg-gray-100/50 transition-colors"
//                           >
...
//                 {/* Action Buttons - ডিজেবল করা হয়েছে */}
...
interface ComboItem {
  ...
  // ✅ backend theke ashe (flattened, populate na kore)
  productName?: string;
  productSlug?: string;
...
interface ProductModalProps {
  product: Product; // ✅ ekhon direct product object pass hobe, id na
  onClose: () => void;
}
...
const ProductModal: React.FC<ProductModalProps> = ({ product, onClose }) => {
  ...
  // Product change hole (naya modal open hole) selectedVariant/UI reset
  useEffect(() => {
  ...
  // Reviews (eta API call thakbei, karon ei data product object-e thake na)
  const { data: reviewsData, isLoading: reviewsLoading, refetch: refetchReviews } = useGetProductReviewsQuery(
    {
      productId,
      limit: REVIEWS_PER_PAGE,
  ...
  // Related products (eta o alada API, product details na — thik ache thakbe)
  const categoryId = product?.categoryID?._id || '';
  const { data: relatedData, isLoading: relatedLoading } = useGetRelatedProductsQuery(categoryId, {
    skip: !categoryId,
  });
  ...
  // ✅ stock check — variant thakle variant-er stock, na thakle product-er stock
  const inStock = selectedVariant ? selectedVariant.stock > 0 : (product?.stock ?? 0) > 0;
  const isCombo = product?.productType === 'combo';
  ...
  const transformedRelatedProducts = relatedProducts.map((p: any) => ({
    ...
    rating: p.rating || 0,
    reviews: p.numReviews || 0,
    categoryName: p.categoryID?.name || p.categoryDetails?.name || '',
    unit: p.unit?.shortName || p.unitDetails?.shortName || '',
    productID: p._id,
    stock: p.stock, // ✅ stock pathalam, na hole ProductCard-e always "in stock" dhore nebe
  }));
          ...
          <div className="flex flex-col md:flex-row md:items-start">
            ...
            <div className="md:w-[55%] p-6 md:p-8">
              <div>
                ...
                {/* Combo Items Section — flattened fields use kora hocche */}
                {isCombo && product.comboItems && product.comboItems.length > 0 && (
                  <div className="mt-4">
                    <div className="flex items-center gap-2 mb-2">
                      <FiPackage className="text-[var(--color-primary)]" size={16} />
