...
const OrderDetailsPage = () => {
  ...
  const timeline = order.statusTimeline || [];
  const progressPct =
    currentStatusIndex < 0 ? 0 : (currentStatusIndex / (statusSteps.length - 1)) * 100;
    ...
    <div className="min-h-screen bg-[var(--color-surface)] pb-24">
      <div className="max-w-4xl mx-auto px-4 pt-6 space-y-4">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              <span
                className={`text-xs font-bold px-3 py-1.5 rounded-full border capitalize ${statusColor[order.status] || 'bg-gray-100 text-gray-600 border-gray-200'}`}
              >
                {order.status.replace('_', ' ')}
              </span>
            </div>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-4 text-xs text-[var(--color-text-muted)]">
            ...
            <span className="w-1 h-1 bg-gray-300 rounded-full" />
            <span
              className={`flex items-center gap-1.5 font-semibold ${paymentStatusColor[order.paymentStatus] || ''}`}
            >
              {order.paymentStatus === 'paid' ? '✓' : '○'}{' '}
              {order.paymentStatus.replace('_', ' ')} • {order.paymentMethod?.toUpperCase()}
            </span>
          </div>
        </div>
          ...
          <div className="relative">
            ...
            <div className="relative flex justify-between">
              {statusSteps.map((step, idx) => {
                const isCompleted = idx < currentStatusIndex;
                const isActive = idx === currentStatusIndex;
                const isPending = idx > currentStatusIndex; // ✅ এখানে isPending ডিফাইন করুন
                const timelineEntry = timeline.find((t: any) => t.status === step.key);
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
          ...
          {/* Table Header */}
          <div className="grid grid-cols-12 gap-2 px-5 py-3 bg-[var(--color-surface)] text-xs font-semibold text-[var(--color-text-muted)] border-b border-gray-100">
          ...
          <div className="divide-y divide-gray-100">
            {order.items.map((item: any, i: number) => (
              <div key={i} className="grid grid-cols-12 gap-2 px-5 py-3 items-center">
                <div className="col-span-5 flex items-center gap-3">
                  <div className="w-12 h-12 rounded-xl border border-gray-200 overflow-hidden bg-[var(--color-surface)] flex-shrink-0">
                    <img
                      src={item.thumbnail}
                      alt={item.productName}
                ...
                <div className="col-span-3 text-center text-sm font-medium text-[var(--color-text-primary)]">
                  {item.quantity}
                </div>
          ...
          {/* Summary Footer */}
          <div className="p-5 border-t border-gray-100 bg-[var(--color-surface)]">
            <div className="max-w-xs ml-auto space-y-2 text-sm">
              <div className="flex justify-between">
                <span className="text-[var(--color-text-muted)]">Subtotal ({order.items.length} items)</span>
                <span className="font-semibold text-[var(--color-text-primary)]">৳{order.subtotal}</span>
              </div>
              {order.couponCode && (
                <div className="flex justify-between">
          ...
          <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
            ...
            <div className="space-y-2.5 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-medium text-[var(--color-text-primary)] font-mono">
                  #{order.orderNumber}
                </span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between pt-2 border-t border-gray-100">
                ...
                <span className="font-bold text-[var(--color-text-primary)]">৳{order.totalAmount}</span>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-3">
              <div>
                ...
                <p className="text-sm text-[var(--color-text-muted)] mt-0.5 flex items-center gap-1">
                  <FiPhone size={13} className="text-[var(--color-text-muted)]" />
                  {order.deliveryAddress.phone}
                </p>
              </div>
              <div className="flex items-start gap-2.5">
                <FiMapPin size={15} className="text-[var(--color-primary)] mt-0.5 flex-shrink-0" />
              ...
              {order.deliveryAddress.deliveryNotes && (
                <p className="text-xs text-[var(--color-text-muted)] italic border-l-2 border-[var(--color-primary)] pl-3">
                  {order.deliveryAddress.deliveryNotes}
                </p>
              )}
            </div>
