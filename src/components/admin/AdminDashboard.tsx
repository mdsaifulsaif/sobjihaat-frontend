...
const RiderDashboardPage = () => {
  ...
  // Stats Cards - Always show TOTAL, range is shown as subtitle
  const stats = [
    {
      title: 'Total Deliveries',
      value: summary?.totalDeliveries || 0, // ✅ Always TOTAL
      icon: FiTruck,
      color: '#3B82F6',
      bgColor: 'rgba(59, 130, 246, 0.12)',
      subtitle: `${summary?.rangeDeliveries || 0} in this ${range}`,
    },
    {
      title: 'Total Earnings',
      value: `৳${earnings?.total || 0}`, // ✅ Always TOTAL
      icon: FiDollarSign,
      color: '#22C55E',
      bgColor: 'rgba(34, 197, 94, 0.12)',
      subtitle: `৳${earnings?.range || 0} in this ${range}`,
    },
    {
  ...
  // Get range label
  const getRangeLabel = () => {
    return rangeOptions.find(o => o.value === range)?.label || 'This Period';
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
          <div className="flex justify-between items-center mb-6">
            <div>
              ...
              <p className="text-xs text-gray-400">
                {getRangeLabel()} earnings breakdown
              </p>
            </div>
            <div className="flex items-center gap-4">
          ...
          {chart?.labels?.length === 0 ? (
            <div className="h-56 flex flex-col items-center justify-center text-gray-400">
              <FiTrendingUp size={40} className="mb-3 text-gray-300" />
              <p className="text-sm font-medium">No data available for this period</p>
              <p className="text-xs text-gray-400 mt-1">
                {range === 'today' 
                  ? 'You have no deliveries today' 
                  : `No deliveries in this ${range} period`}
              </p>
            </div>
          ) : (
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
          ...
          <div className="mt-4 p-4 bg-gray-50 rounded-xl border border-gray-100">
            ...
            <div className="flex justify-between items-center text-sm mt-1">
              <span className="text-gray-500">This {getRangeLabel()}</span>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400">
              {recentOrders.length > 0 
                ? `Latest ${recentOrders.length} deliveries` 
                : 'No recent orders'}
            </p>
          </div>
          <Link
        ...
        {recentOrders.length === 0 ? (
          <div className="p-12 text-center">
            ...
            <p className="text-xs text-gray-400 mt-1">You have no recent orders</p>
          </div>
        ) : (
