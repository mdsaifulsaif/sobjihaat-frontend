...
const CategoryPage = () => {
  ...
  return (
    <div className="w-full px-3 md:px-4 py-6">
      ...
      {/* Product Grid - Fixed Card Width */}
      {/* {products.length > 0 && (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-4 md:gap-5 justify-start">
          {products.map((product: any) => (
            <div 
              key={product._id} 
              className="w-full max-w-[175px] sm:max-w-[185px] md:max-w-[205px] lg:max-w-[218px]"
            >
              <ProductCard
                product={{
                  id: product._id,
...
{products.length > 0 && (
  <div className="flex flex-wrap justify-start gap-3 md:gap-3">
    {products.map((product: any) => (
      <div 
        key={product._id} 
        ...
        */
        className="w-[calc(50%-6px)] sm:w-[calc(50%-6px)] md:w-[170px] lg:w-[170px] flex-shrink-0"
      >
        <ProductCard
          product={{
            id: product._id,
